...
import { Profile, ResponseGetGallery } from "@/types/api"
...
export function ProfileGallery({ profile }: { profile: Profile }) {
	...
	const { data: images, isPending, fetchNextPage, error } = useInfiniteQuery<ResponseGetGallery>({
		queryKey: ["profile-images", profile.AccountId],
		...
		queryFn: async ({ pageParam = null }) => {
			...
			const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/medias/account?${query.toString()}`, {
				credentials: "include"
			})
